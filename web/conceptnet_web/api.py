...
limiter = Limiter(
    app, key_func=get_remote_address, default_limits=["600 per minute", "6000 per hour"]
)
CORS(app)
...
def query_standardize_uri():
    ...
    return jsonify(
        {"@context": responses.CONTEXT, "@id": standardized_concept_uri(language, text)}
    )
...
def error_page_not_found(e):
    return render_error(404, "%r isn't a URL that we understand." % flask.request.path)
